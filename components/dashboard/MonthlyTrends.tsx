...
export function MonthlyTrends() {
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      return (
        <div className="bg-white dark:bg-[#1A2A3A] p-3 sm:p-4 rounded-lg shadow-card-dark border border-gray-100 dark:border-gray-700">
          <p className="font-semibold text-navy dark:text-white text-sm sm:text-base mb-1 sm:mb-2">{label}</p>
          {payload.map((item: any) => (
            <p key={item.name} className="text-xs sm:text-sm dark:text-gray-300" style={{ color: item.color }}>
  ...
  if (loading) {
    return (
      <div className="bg-white dark:bg-[#1A2A3A] rounded-xl shadow-card-dark p-4 sm:p-6">
        <div className="flex items-center justify-between mb-3 sm:mb-4">
          <div>
            <h3 className="text-base sm:text-lg font-semibold text-navy dark:text-white">Monthly Trends</h3>
  ...
  return (
    <div className="bg-white  rounded-xl shadow-card-dark p-4 sm:p-6 transition-all duration-300 hover:shadow-card-hover">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-3 sm:mb-4">
        <div>
          <h3 className="text-base sm:text-lg font-semibold text-navy dark:text-white">Monthly Trends</h3>
          <p className="text-[10px] sm:text-xs text-gray-400 dark:text-gray-400 mt-0.5">
        ...
        {data.length > 0 && (
          <button
            ...
            className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-1.5 sm:py-2 bg-electric-blue text-white text-xs sm:text-sm font-medium rounded-lg hover:bg-electric-blue/90 transition-colors shadow-md shadow-electric-blue/30 dark:bg-electric-blue dark:hover:bg-electric-blue/90"
          >
      ...
      {data.length === 0 ? (
        <div className="h-48 sm:h-64 flex flex-col items-center justify-center text-gray-400 text-sm sm:text-base">
          <Wallet className="w-8 h-8 mb-2 text-gray-300 dark:text-gray-600" />
          No data available
        ...
        <>
          ...
          {/* Quick Stats Summary */}
          <div className="grid grid-cols-3 gap-2 sm:gap-4 mt-4 sm:mt-6 pt-3 sm:pt-4 border-t border-gray-100 dark:border-gray-700">
      ...
      {/* Analytics Modal - Dark Theme Support */}
      {showAnalytics && bestMonth && worstMonth && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 animate-fade-in" onClick={() => setShowAnalytics(false)}>
          <div className="bg-white dark:bg-[#1A2A3A] rounded-xl shadow-card-dark max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6 sm:p-8 animate-scale-in" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-lg sm:text-xl font-bold text-navy dark:text-white">Monthly Trends Analytics</h3>
              <button 
            ...
            <div className="space-y-4">
              ...
              <div className="p-4 bg-emerald-50 dark:bg-emerald-900/30 rounded-lg border border-emerald-200 dark:border-emerald-800">
                ...
                <p className="text-sm text-gray-700 dark:text-gray-300 mt-1">
                  <span className="font-medium dark:text-white">{bestMonth.month}</span> - Saved 
                  <span className="font-semibold text-emerald-600 dark:text-emerald-400"> GH₵{bestMonth.savings.toFixed(2)}</span>
              ...
              <div className="p-4 bg-rose-50 dark:bg-rose-900/30 rounded-lg border border-rose-200 dark:border-rose-800">
                ...
                <p className="text-sm text-gray-700 dark:text-gray-300 mt-1">
                  <span className="font-medium dark:text-white">{worstMonth.month}</span> - Saved 
                  <span className="font-semibold text-rose-600 dark:text-rose-400"> GH₵{worstMonth.savings.toFixed(2)}</span>
                  <span className="text-gray-500 dark:text-gray-400 text-xs block mt-0.5">
                ...
                <div className="space-y-1.5 mt-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-gray-700 dark:text-gray-300">
                      <span className="font-medium dark:text-white">Income</span> - Money coming in
                    </span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="w-4 h-4 rounded bg-rose-500"></div>
                    <span className="text-xs text-gray-700 dark:text-gray-300">
                      <span className="font-medium dark:text-white">Expenses</span> - Money going out
                    </span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="w-4 h-4 rounded bg-blue-500"></div>
                    <span className="text-xs text-gray-700 dark:text-gray-300">
                      <span className="font-medium dark:text-white">Savings</span> - Income - Expenses
                    </span>
                  </div>
                </div>
              </div>
              ...
              {/* Key Insights */}
              <div className="p-4 bg-gray-50 dark:bg-white/5 rounded-lg border border-gray-200 dark:border-gray-700">
                <h4 className="text-sm font-semibold text-navy dark:text-white flex items-center gap-2">
                  Key Insights
                </h4>
                <ul className="space-y-1.5 mt-2">
                  <li className="text-xs text-gray-700 dark:text-gray-300 flex items-start gap-2">
                    <span className="text-emerald-500">•</span>
                    Total Income: <span className="font-medium dark:text-white">GH₵{totals.income.toFixed(2)}</span>
                  </li>
                  <li className="text-xs text-gray-700 dark:text-gray-300 flex items-start gap-2">
                    <span className="text-rose-500">•</span>
                    Total Expenses: <span className="font-medium dark:text-white">GH₵{totals.expenses.toFixed(2)}</span>
                  </li>
                  <li className="text-xs text-gray-700 dark:text-gray-300 flex items-start gap-2">
                    <span className="text-blue-500">•</span>
                    Total Savings: <span className="font-medium dark:text-white">GH₵{totals.savings.toFixed(2)}</span>
                  </li>
                  <li className="text-xs text-gray-700 dark:text-gray-300 flex items-start gap-2">
                    <span className="text-purple-500">•</span>
                    Savings Rate: <span className="font-medium dark:text-white">{totals.savingsRate.toFixed(1)}%</span>
                  </li>
                </ul>
              </div>
            </div>
            ...
            <button
              onClick={() => setShowAnalytics(false)}
              className="w-full mt-6 py-2.5 bg-navy dark:bg-[#3B82F6] text-white rounded-lg hover:bg-navy/90 dark:hover:bg-[#3B82F6]/90 transition-colors text-sm font-medium"
            >
              Close Analytics
            </button>
          </div>
