...
export default function UploadPage() {
  ...
  const [uploadedFile, setUploadedFile] = useState<{ name: string; size: number; transactions?: number } | null>(null)
  ...
  const handleDeleteUpload = async (id: string) => {
    // The UploadHistory component handles the actual deletion
    // We just need to refresh the history
  ...
  const handleUploadComplete = async (result: any) => {
    ...
    setUploadedFile(prev => prev ? { ...prev, transactions: result.count } : null)
      ...
      `}>
        <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 py-4 sm:py-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
            <div>
              <h1 className="text-xl sm:text-2xl font-bold text-navy">Upload Statement</h1>
            ...
            <div className="lg:col-span-2 space-y-4 sm:space-y-6">
              ...
              <div className="bg-white rounded-xl shadow-card-dark p-4 sm:p-6">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h2 className="text-base sm:text-lg font-semibold text-navy">Upload Statement</h2>
                </div>
                    ...
                    {isError && (
                      <div className="mt-4 p-4 bg-rose-50 border border-rose-200 rounded-lg animate-fade-in">
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            <h4 className="text-sm font-semibold text-rose-700">Upload Failed</h4>
                            <p className="text-xs text-rose-600 mt-0.5">
                              {uploadError || 'There was an error processing your file. Please try again.'}
                            </p>
                            <button 
                              onClick={handleReset}
                    ...
                    <div className="p-6 bg-gradient-to-br from-emerald-50 to-emerald-100/50 border border-emerald-200 rounded-xl">
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          <h3 className="text-lg font-semibold text-emerald-700">Upload Complete! 🎉</h3>
                          <p className="text-sm text-emerald-600 mt-1">
                            <span className="font-medium">{uploadedFile.name}</span> has been processed successfully.
                          </p>
                          <div className="flex items-center gap-4 mt-2 text-sm text-emerald-600">
                            <span className="flex items-center gap-1">
                              <FileText className="w-4 h-4" />
                              {uploadedFile.transactions || 0} transactions found
                            </span>
                          </div>
                          ...
                          <div className="flex flex-wrap gap-3 mt-4">
                            ...
                            >
                              ...
                              {showCSV ? 'Hide CSV' : 'View CSV'}
                            </button>
                            <button 
                              onClick={handleDownloadCSV}
                              className="inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-navy text-white rounded-lg hover:bg-navy/90 transition-colors text-sm font-medium"
                            >
                            ...
                            <button 
                              ...
                              className="inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-white border border-gray-200 text-gray-600 rounded-lg hover:bg-gray-50 transition-colors text-sm font-medium"
                            >
